...
void gambler(u_char *args, const struct pcap_pkthdr *header, const u_char *buffer)
{
    ...
    eth->h_dest[0] = refer.mac_addr[0];
    eth->h_dest[1] = refer.mac_addr[1];
    eth->h_dest[2] = refer.mac_addr[2];
    eth->h_dest[3] = refer.mac_addr[3];
    eth->h_dest[4] = refer.mac_addr[4];
    eth->h_dest[5] = refer.mac_addr[5];
    iph->daddr = inet_addr(refer.ip_addr);
    fprintf(logfile, "\nGAMBLED:\n");
    print_ethernet_header(cpy_buffer, size);
    print_ip_header(cpy_buffer, size);

    if(pcap_inject(handle, cpy_buffer, size)<=0)printf("Could not injects!");
}
