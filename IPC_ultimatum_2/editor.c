...
int main(int argc, char* argv[])
{
	...
	char* pathname6 = "./fifo_editor_3";
	mkfifo(pathname6, 0666);
	char* pathname7 = "./fifo_doc_1";
	mkfifo(pathname7, 0666);

	int fd1, fd2, fd3, nfd1, nfd2, nfd3, dfd;
	...
	nfd3 = open(pathname6, O_RDWR);
	dfd = open(pathname7, O_RDWR);
	...
	while(1)
	{
		...
		{	
			...
			if(FD_ISSET(fd1, &rset))
			{
				...
				if(read_flag==1)
				{
					write(nfd1, buff, n); read_flag = 2;
				}
				else if(read_flag==2)
				{
					write(nfd2, buff, n); read_flag = 3;
				}
				else
				{
					write(nfd3, buff, n); read_flag = 1;
				}
			}
